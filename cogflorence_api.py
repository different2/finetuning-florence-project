#!/usr/bin/env python3
# cogflorence_api.py v2 - Added /refresh-analysis endpoint
import os
...
# ---------- helper: run caption task (memory optimized, FP32) ----------
def run_caption_task(image: Image.Image, task_prompt: str, max_tokens: int = 1024) -> str:
    # Wrap in no_grad to prevent storing activations
    with torch.no_grad():
        # Process single image only
        inputs = PROCESSOR(text=task_prompt, images=image, return_tensors="pt")
        inputs = move_batch_to_device(inputs, DEVICE, TORCH_DTYPE)
        # Generate text
        ...
        # Optional: clear cached memory (MPS / CUDA)
        if DEVICE.type == "mps":
            torch.mps.empty_cache()
        elif DEVICE.type == "cuda":
            torch.cuda.empty_cache()
        # Try processor parsing
        try:
            parsed = PROCESSOR.post_process_generation(
                text, task=task_prompt, image_size=(image.width, image.height)
            )
            return parsed.get(task_prompt, text)
        except Exception:
            return text
...
async def detect_objects(request: ObjectDetectionRequest):
    ...
    try:
        ...
        with torch.no_grad():  # no gradients stored
            caption_text = run_caption_task(image, caption_prompt)
        
        logger.info("Caption text: %s", caption_text)
...
@app.post("/refresh-analysis", response_model=RefreshAnalysisResponse)
async def refresh_analysis(request: RefreshAnalysisRequest):
    """
    Re-runs Florence-2 phrase grounding generation with awareness of user's annotations.
    Simulates what a fine-tuned model would generate for CAPTION_TO_PHRASE_GROUNDING task.
    """
    if MODEL is None or PROCESSOR is None:
        raise HTTPException(status_code=503, detail="Model not loaded")
    ...
    try:
        image_bytes = base64.b64decode(request.image_b64)
        image = Image.open(BytesIO(image_bytes)).convert("RGB")
        logger.info("Refresh analysis for image size: %s", (image.width, image.height))
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Invalid base64 image: {e}")

    try:
        # Step 1: Generate fresh <MORE_DETAILED_CAPTION> as base
        detailed_caption = run_caption_task(image, "<MORE_DETAILED_CAPTION>", max_tokens=1024)
        logger.info("Base detailed caption: %s", detailed_caption)
        
        # Step 2: Build context from user annotations
        annotation_labels = []
        annotation_descriptions = []
        
        for ann in request.annotations:
            if ann.label and ann.label != "new annotation":
                annotation_labels.append(ann.label)
                if ann.description:
                    annotation_descriptions.append(ann.description)
        
        # Step 3: Enhance caption with user's vocabulary and notes
        phrase_caption = detailed_caption
        
        # If user provided notes, incorporate them
        if request.user_notes:
            phrase_caption += f" {request.user_notes}"
        
        # Add metadata context
        metadata_parts = []
        if request.style:
            metadata_parts.append(f"{request.style} style")
        if request.source:
            metadata_parts.append(f"from {request.source}")
        if request.artist:
            metadata_parts.append(f"by {request.artist}")
        
        if metadata_parts:
            phrase_caption += f" This is {', '.join(metadata_parts)}."
        
        logger.info("Phrase grounding caption: %s", phrase_caption)
        
        return RefreshAnalysisResponse(
            phrase_grounding_caption=phrase_caption
        )

    except Exception as e:
        logger.exception("Error during refresh_analysis: %s", e)
        raise HTTPException(status_code=500, detail=f"Error during refresh analysis: {e}")
